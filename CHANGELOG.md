...
- 界面文案全面改为面向使用者的表述：采集说明、频率与资源面板、公众号商业化支持设置、进度与状态提示不再出现接口名、内部标识、错误码、配额公式与调度术语；"抓取"统一改为"采集"。
- 同步到 GitHub 的归档仓库 README、仓库描述与 RSS 订阅源中的来源署名统一使用产品名「微探」。

### Fixed

- 授权服务未就绪时的提示改为面向使用者的重启指引，不再输出开发环境信息。
...
- 修复外部公众号抓取回退到受限后台列表接口的回归：`searchbiz` 只负责解析稳定 `fakeid`，自有号读取后台发表记录，外部号通过公开索引发现候选，并在写入前用文章内 `biz` 精确核验归属。
- 公共索引为空时不再回退微信后台列表，也不再继承或延长旧接口的一小时本地冷却；重复勾选“抓正文”不会因此制造新的 `ret=200013` 循环。
- 统一识别文章页中的 `biz`、`__biz` 和 `bizuin` 身份字段；已确认归属错误的历史候选会从所选账号移除，无法确认身份时保留既有数据。
- 新增不含 token、Cookie 或完整签名链接的 `[DEBUG]` 链路日志，用于区分公共索引、后台来源、本地保护、resident sidecar 版本与实际请求时序。
- Windows 发布 ZIP 会同时包含桌面主程序和必需的 `wcx.exe` sidecar；macOS 增量包会按上一版本的真实资产名取包，不再稳定退化为空增量清单。
...
- 管理员可在已授权账号列表中编辑或清空客户备注，保存后会回读云端记录并刷新授权码缓存。

## 0.3.9 - 2026-08-31

### Added

- 管理员可在已授权账号列表中查看、复制和收起完整激活码；签发前会再次验证当前 Supabase 管理员身份。

### Fixed

- 管理员写入云端授权后会使用客户端相同的 RPC 回读确认，不再把无法被客户端读取的记录提示为“已生效”。
- 远程同步未找到当前账号的有效授权时，会明确提示管理员核对邮箱、用户 ID、状态和到期时间。
